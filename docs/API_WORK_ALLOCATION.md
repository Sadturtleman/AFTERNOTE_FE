...
### 1-1. 모든 API의 DTO → Repository (29개 전체)
...
> Auth는 중요도가 높아, **DTO→Repo(1-1) + UseCase→ViewModel+UIModel** 까지 한 명이 담당.
...
> User API도 스웨거 등록 완료로, **DTO→Repo(1-1) + UseCase→ViewModel+UIModel** 까지 한 명이 담당.
...
**담당 계층**: **Domain + UI**  
**전제**: Time-Letters `Repository`(인터페이스)와 `RepositoryImpl`은 정일혁이 DTO→Repo 작업에서 이미 구현.
...
**담당 계층**: **Domain + UI**  
**전제**: Received `Repository`/`RepositoryImpl`은 정일혁이 DTO→Repo에서 구현.
...
**담당 계층**: **Domain + UI**  
**전제**: Mind-Record `Repository`/`RepositoryImpl`은 정일혁이 DTO→Repo에서 구현.
...
**담당 계층**: **Domain + UI**  
**전제**: Afternote `Repository`/`RepositoryImpl`은 정일혁이 DTO→Repo에서 구현.
...
|------|-----------------------------------------------|-----------------------------------|------------------------------|
| **정일혁** | 29개 API 전체 | Auth 8개, User 4개 (UseCase·Model) | Auth 8개, User 4개 (ViewModel·UIModel) |
