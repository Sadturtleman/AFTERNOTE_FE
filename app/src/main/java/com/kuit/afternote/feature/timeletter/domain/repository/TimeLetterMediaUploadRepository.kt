...
/**
 * 타임레터 첨부 미디어 업로드 (POST /files/presigned-url 후 S3 PUT).
 *
 * content URI 문자열을 받아 업로드 후 사용할 fileUrl을 반환합니다.
 */
interface TimeLetterMediaUploadRepository {
    ...
    /**
     * [uriString]에 해당하는 오디오를 S3에 업로드하고, 타임레터 mediaList에 넣을 fileUrl을 반환합니다.
     *
     * @param uriString content URI 문자열 (예: content://media/...)
     * @return 성공 시 업로드된 오디오 URL, 실패 시 Result.failure
     */
    suspend fun uploadAudio(uriString: String): Result<String>
}
